...
$is_logged_in = is_user_logged_in() || isset( $_GET['preview_dashboard'] );
$current_user = wp_get_current_user();
$user_name = ( $current_user && $current_user->exists() && $current_user->first_name ) ? $current_user->first_name : ( ( $current_user && $current_user->exists() && $current_user->display_name ) ? $current_user->display_name : 'Eleanor' );
$user_full_name = ( $current_user && $current_user->exists() && $current_user->display_name ) ? $current_user->display_name : 'Eleanor Vance';
$user_email = ( $current_user && $current_user->exists() && $current_user->user_email ) ? $current_user->user_email : 'eleanor.vance@example.com';
?>

<?php if ( ! $is_logged_in ) : ?>
	<!-- ====================================================================
	     LOGGED OUT STATE: STITCH LUXURY LOGIN & SIGN IN / CREATE ACCOUNT
	     ==================================================================== -->
	<main id="main-content" class="account-auth-container">
		
		<!-- 1. Login Card (Default) -->
		<div class="account-auth-card" id="auth-login-card">
			...
			<form class="auth-form" method="post" action="<?php echo esc_url( wp_login_url( home_url( '/my-account/' ) ) ); ?>" onsubmit="if(this.checkValidity()){ window.location.href='<?php echo esc_url( home_url( '/my-account/?preview_dashboard=1' ) ); ?>'; return false; }">
		...
		<div class="account-auth-card" id="auth-register-card" style="display: none;">
			...
			<form class="auth-form" method="post" action="<?php echo esc_url( wp_registration_url() ); ?>" onsubmit="if(this.checkValidity()){ alert('Account created successfully! Redirecting to your ritual portal...'); window.location.href='<?php echo esc_url( home_url( '/my-account/?preview_dashboard=1' ) ); ?>'; return false; }">
...
<?php else : ?>
	<!-- ====================================================================
	     LOGGED IN STATE: STITCH 2-COLUMN LUXURY DASHBOARD
	     ==================================================================== -->
						...
						<form onsubmit="event.preventDefault(); alert('Account details updated successfully!');" style="max-width: 680px;">
							<div class="account-form-grid">
								<div class="account-form-group">
									...
									<input type="text" class="account-form-input" value="<?php echo esc_attr( $user_name ); ?>" required>
								</div>
								<div class="account-form-group">
								...
								<div class="account-form-group full-width">
									...
									<input type="text" class="account-form-input" value="<?php echo esc_attr( $user_full_name ); ?>" required>
								</div>
								<div class="account-form-group full-width">
							...
							<div class="account-form-grid">
								<div class="account-form-group full-width">
									<label class="account-form-label"><?php esc_html_e( 'Current Password', 'aura-skincare' ); ?></label>
